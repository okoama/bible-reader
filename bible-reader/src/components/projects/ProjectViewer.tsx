...
export default function ProjectViewer({ projectId, refreshKey, onBack, onEdit, onDelete, onStatusChange, onNavigateToReference }: Props) {
  ...
  const handleSaveNotes = useCallback(async () => {
    ...
    await projectRepo.save(updated);
    setProject(updated);
    setNotesSaved(true);
    setSaving(false);
        ...
        <SectionCard title="Notes & Highlights">
          {showNewNote ? (
            ...
            <>
              ...
              {notes.length > 0 && <button type="button" onClick={() => setShowNewNote(true)} className="mt-2 text-xs text-accent hover:underline">+ Add Note</button>}
            </>
          )}
        </SectionCard>
