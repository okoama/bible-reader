...
export default function ProjectPicker({ value, onChange }: ProjectPickerProps) {
  ...
  useEffect(() => {
    let active = true;
    void repo.findAll().then((all) => { if (active) setProjects(all); }).finally(() => { if (active) setLoading(false); });
    return () => { active = false; };
  }, []);

  return (
    <div>
      <label htmlFor="project-picker" className="mb-1 block text-xs font-medium opacity-60">Project</label>
      <select
        id="project-picker"
        value={value ?? ''}
